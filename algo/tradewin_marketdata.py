...
from tradewin_config import TradewinLogger
from tradewin_strategy import IndicatorCalculator, StrategyApplier, VWAPStrategy, ORBStrategy
...
class MarketData:
    ...
    def decide_trade_from_row(self, row):
        ...
        # Skip if not valid
        if not result.valid:
            logger.info(f"Decision rejected — {result.reason}")
            return result
