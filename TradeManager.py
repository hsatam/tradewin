...
class TradeManager:
    ...
    def place_order(self, trade_date: datetime, action: str, price: float, stoploss: float, strategy: str,
        ...
        # TradeWinUtils.log_trade(self.position, self.entry_price)
    ...
    def _handle_buy_trailing_sl(self, trade_date, current_price: float):
        ...
        self._maybe_update_sl(trade_date, candidate)
    ...
    def _maybe_update_sl(self, trade_date, new_sl: float):
        ...
        if new_sl_rounded == self.stop_loss or abs(new_sl_rounded - self.stop_loss) < 0.01:
            logger.debug("Trailing SL unchanged at %s", self.stop_loss)
            return
        ...
        message = f"Trailing SL adjusted to {self.stop_loss:.2f}"
        logger.info(message)
